  ...
  return (
    <div className="glass float relative rounded-lg p-5" aria-hidden>
      <div className="flex items-center justify-between">
        <span className="font-mono text-[11px] text-stone-500">amicited / check</span>
      ...
      <div className="mt-4 flex items-center gap-4">
        <svg width="88" height="88" viewBox="0 0 88 88" className="-rotate-90 shrink-0">
          <circle cx="44" cy="44" r={r} stroke="#E7E2DB" strokeWidth="9" fill="none" />
          <circle
            cx="44"
            cy="44"
            r={r}
        ...
        <div>
          ...
          <p className="mt-0.5 font-mono text-[11px] text-stone-500">cited by 3 of 4 engines</p>
        </div>
      </div>

      <ul className="mt-4 space-y-2 border-t border-stone-200/70 pt-4">
        {ENGINES.map((e, i) => (
          <li key={e.name} className="flex items-center gap-2.5">
            <span className="w-16 shrink-0 truncate text-xs text-stone-600">{e.name}</span>
            <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-stone-200/70">
              <div
                ...
                style={{
                  ...
                  background: e.cited ? '#059669' : '#E11D48',
                }}
              />
            </div>
            <span className="w-9 shrink-0 text-right font-mono text-[11px] tabular-nums text-stone-500">
              {e.pct}%
            </span>
          </li>
        ))}
      ...
      <div className="mt-4 rounded border border-stone-200/70 bg-white/50 p-3">
        <p className="font-mono text-[10px] uppercase tracking-wide text-stone-500">named instead</p>
        <ul className="mt-2 space-y-1.5">
          {COMPETITORS.map((cm) => (
            <li key={cm.name} className="flex items-center gap-2.5">
              <span className="w-14 shrink-0 text-xs text-stone-700">{cm.name}</span>
              <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-stone-200/70">
