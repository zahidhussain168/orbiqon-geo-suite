  ...
  return (
    <section className="space-y-5">
      ...
      <div className="card animate-fade-up p-6 sm:p-8">
        <div className="flex flex-col items-center gap-7 sm:flex-row sm:gap-10">
          ...
          <div className="min-w-0 flex-1 text-center sm:text-left">
            ...
            <h1 className="mt-0.5 truncate text-3xl font-bold tracking-tight text-slate-900">
              {result.brand}
            </h1>
            <p className="mt-2.5 text-sm text-slate-600">
              Cited by <span className="font-semibold text-slate-900">{citedEngines} of {shown.length}</span>{' '}
      ...
      <div className="flex items-start gap-2.5 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
        ...
        <p>
          <span className="font-medium text-slate-700">How to read this:</span> AI answers vary run
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {ok.map((e) => {
            ...
            return (
              <button
                ...
                className={`chip transition ${on ? 'border-slate-300 bg-white text-slate-800 shadow-sm' : 'opacity-50'}`}
                aria-pressed={on}
              >
        ...
        <div className="inline-flex self-start rounded-lg border border-slate-200 bg-slate-50 p-0.5 text-xs sm:self-auto">
    ...
    <div className="card h-full p-4">
      ...
      <p className="mt-3 text-sm text-slate-600">
        ...
        {engine.bestPosition != null && (
          <>
            {' '}
            · best position <span className="font-semibold tabular-nums text-slate-900">#{engine.bestPosition}</span>
          </>
        )}
      </p>
      ...
      <div className="mt-3 flex items-center gap-3">
        ...
        <span className="w-9 text-right text-xs font-medium tabular-nums text-slate-500">{pct}%</span>
      </div>
          ...
          {sources.map((s, i) => (
            <span key={s.domain}>
              ...
              <a
                ...
                className="font-medium text-slate-600 underline decoration-slate-300 underline-offset-2 hover:text-brand-700"
              >
                {s.domain}
              </a>
            </span>
            ...
            <div className="mt-2 space-y-2 border-t border-slate-100 pt-3">
              {examples.map((s, i) => (
                <div key={i} className="rounded-lg bg-slate-50 p-2.5 text-xs">
                  <p className="mb-1 text-slate-400">“{s.prompt}”</p>
                  <p className="leading-relaxed text-slate-600">{highlight(s.text, brand)}</p>
                </div>
              ))}
            </div>
          )}
    ...
    <div className="card h-full border-dashed p-4">
      <div className="flex items-center justify-between">
        <span className="flex items-center gap-2 font-medium text-slate-600">
          <span className="grid h-6 w-6 place-items-center rounded-md bg-slate-100 text-[11px] font-bold text-slate-400">
            {engine.displayName[0]}
          </span>
          {engine.displayName}
        </span>
        ...
        <tbody>
          {result.prompts.map((prompt) => (
            <tr key={prompt} className="border-t border-slate-100">
              <td className="max-w-[240px] truncate py-2.5 pr-3 text-slate-700" title={prompt}>
                {prompt}
              </td>
              {shown.map((e) => {
                    ...
                    {cell?.cited ? (
                      <span className="inline-flex items-center gap-1 text-emerald-600">
                        ...
                        {cell.bestPosition != null && (
                          <span className="text-xs tabular-nums text-slate-400">#{cell.bestPosition}</span>
                        )}
                      </span>
                    ) : (
    ...
    <ul className="mt-4 space-y-2.5">
      {competitors.map((c, i) => (
        <li key={c.name} className="flex items-center gap-3">
          ...
          <span
            className={`w-28 shrink-0 truncate text-sm ${i === 0 ? 'font-semibold text-slate-900' : 'text-slate-700'}`}
            title={c.name}
          >
            {c.name}
          </span>
